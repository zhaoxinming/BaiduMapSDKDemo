...
import com.baidu.mapapi.animation.AnimationOrder;
 ...
 * 注：自763版本起，Marker和Animation升级为overlay2.0版本。后续1.0版本只做维护，不再进行功能升级
 */
public class MarkerAnimationDemo extends AppCompatActivity {
    ...
    public Animation getSingleScaleAnimation() {
        SingleScaleAnimation mSingleScale =
                new SingleScaleAnimation(SingleScaleAnimation.ScaleType.SCALE_X, 1f, 2f);
    ...
    public void startAnimationSet() {
        ...
        animationSet.addAnimation(getAlphaAnimation(), AnimationOrder.ANIM_ORDER_WITH);
        animationSet.addAnimation(getRotateAnimation(),AnimationOrder.ANIM_ORDER_WITH);
        animationSet.addAnimation(getSingleScaleAnimation(),AnimationOrder.ANIM_ORDER_WITH);
        animationSet.addAnimation(getScaleAnimation(),AnimationOrder.ANIM_ORDER_WITH);
    ...
    private Animation getTransformation() {
        ...
        Transformation transformation = new Transformation(latLngA, latLngB);
    ...
    private Animation getTransformationPoint() {
        if (null != mScreenCenterPoint) {
            ...
            Transformation transformation = new Transformation(mScreenCenterPoint, pointTo);
            transformation.setDuration(500);
            transformation.setRepeatMode(Animation.RepeatMode.RESTART);// 动画重复模式
            transformation.setRepeatCount(1);// 动画重复次数
            transformation.setAnimationListener(new Animation.AnimationListener() {
