...
public class CircleDemo extends AppCompatActivity implements SeekBar.OnSeekBarChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mClickCircle=(CheckBox) findViewById(R.id.clickcirlce);
        ...
        // 点击polyline的事件响应
        mBaiduMap.setOnCircleClickListener(new BaiduMap.OnCircleClickListener() {
            @Override
            public boolean onCircleClick(Circle circle) {
                if (circle == mCircle) {
                    mCircle.remove();
                } else if (circle == mCircleTwo) {
                    mCircleTwo.remove();
                } else if (circle == mCircleThree){
                    mCircleThree.remove();
                }
                return false;
            }
     ...
     * 设置Circle是否可点击
     */
    public void setCircleClick(View view) {
        if (mCircle == null || mCircleTwo == null || mCircleThree == null) {
            return;
        }
        CheckBox checkBox = (CheckBox) view;
        if (checkBox.isChecked()){
            mCircle.setClickable(true);
            mCircleTwo.setClickable(true);
            mCircleThree.setClickable(true);
        } else {
            mCircle.setClickable(false);
            mCircleTwo.setClickable(false);
            mCircleThree.setClickable(false);
        }
    }
    ...
    public void initCircle() {
        ...
        OverlayOptions ooCircleA = new CircleOptions()
                ...
                .radius(1400).bloomType(LineBloomType.GradientA).lineBloomDirection(
                        LineBloomDirection.BloomUp).bloomAlpha(255).setBloomGradientASpeed(5).bloomWidth(mStrokeWidth * 2); //
        //  圆半径，单位：米
    ...
    public void addHole(View view){
        ...
        LatLng llCircleA = new LatLng(39.90923, 116.447428);
        HoleOptions holeOptionsA =
                new CircleHoleOptions().center(llCircleA).radius(1000);
        // 设置Circle的镂空形状选项
        mCircle.setHoleOption(holeOptionsA);
        // 设置Circle的镂空是否可以点击
        mCircle.setHoleClickable(false);

        LatLng llCircleB = new LatLng(39.97923, 116.357428);
        HoleOptions holeOptionsB =
                new CircleHoleOptions().center(llCircleB).radius(1800);
        mCircleTwo.setHoleOption(holeOptionsB);
        mCircleTwo.setHoleClickable(false);
    }
