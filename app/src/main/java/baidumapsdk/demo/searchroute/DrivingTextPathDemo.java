...
import com.baidu.mapapi.overlayutil.TextPathOverlay;
...
public class DrivingTextPathDemo extends AppCompatActivity
        ...
        mSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                switch (position) {
                    case 0:
                        // 时间优先策略，  默认时间优先
                        mDrivingRoutePlanOption.policy(
                                DrivingRoutePlanOption.DrivingPolicy.ECAR_TIME_FIRST);
                        break;
                    case 1:
                        // 躲避拥堵策略
                        mDrivingRoutePlanOption.policy(
                                DrivingRoutePlanOption.DrivingPolicy.ECAR_AVOID_JAM);
                        break;
                    case 2:
                        // 最短距离策略
                        mDrivingRoutePlanOption.policy(
                    ...
                    case 3:
                        // 费用较少策略
                        mDrivingRoutePlanOption.policy(
                                DrivingRoutePlanOption.DrivingPolicy.ECAR_FEE_FIRST);
                        break;
                    default:
                        break;
                }
    ...
    public void searchButtonProcess(View v) {
        ...
        // 设置起终点信息 起点参数
        PlanNode startNode =
                PlanNode.withCityNameAndPlaceName(mEditStartCity.getText().toString().trim(),
                        mStrartNodeView.getText().toString().trim());
        // 终点参数
        PlanNode endNode =
                PlanNode.withCityNameAndPlaceName(mEditEndCity.getText().toString().trim(),
                        mEndNodeView.getText().toString().trim());

        // 是否开起路况默认不开启
        if (mTrafficPolicyCB.isChecked()) {
            // 开启路况
            mDrivingRoutePlanOption.trafficPolicy(
                    DrivingRoutePlanOption.DrivingTrafficPolicy.ROUTE_PATH_AND_TRAFFIC);
        } else {
            // 关闭路况
            mDrivingRoutePlanOption.trafficPolicy(
                    DrivingRoutePlanOption.DrivingTrafficPolicy.ROUTE_PATH);
        }
    ...
    public void onGetDrivingRouteResult(DrivingRouteResult result) {
        if (result != null && result.error == SearchResult.ERRORNO.AMBIGUOUS_ROURE_ADDR) {
            ...
            Toast.makeText(DrivingTextPathDemo.this,
                    "起终点或途经点地址有岐义,通过 result.getSuggestAddrInfo()接口获取建议查询信息",
                    Toast.LENGTH_SHORT).show();
            return;
        }
        if (result == null || result.error == SearchResult.ERRORNO.RESULT_NOT_FOUND) {
            Toast.makeText(DrivingTextPathDemo.this, "抱歉，未找到结果", Toast.LENGTH_SHORT)
                    .show();
            return;
        }
            ...
            if (result.getRouteLines().size() > 1) {
                ...
                if (!hasShowDialog) {
                    ...
                    SelectRouteDialog selectRouteDialog =
                            new SelectRouteDialog(DrivingTextPathDemo.this,
                    ...
                    selectRouteDialog.setOnItemInDlgClickLinster(
                            new SelectRouteDialog.OnItemInDlgClickListener() {
                                public void onItemClick(int position) {
                                    // 获取选中的路线
                                    mRouteLine = mDrivingRouteResult.getRouteLines().get(position);
                                    TextPathOverlay
                                            overlay =
                                            new DrivingTextPathDemo.MyDrivingTextPathOverlay(
                                                    mBaidumap);
                                    mBaidumap.setOnMarkerClickListener(overlay);
                                    mRouteOverlay = overlay;
                                    overlay.setData(
                                            mDrivingRouteResult.getRouteLines().get(position));
                                    overlay.addToMap();
                                    overlay.zoomToSpan();
                                }
                ...
                TextPathOverlay overlay =
                        new DrivingTextPathDemo.MyDrivingTextPathOverlay(mBaidumap);
                mRouteOverlay = overlay;
                mBaidumap.setOnMarkerClickListener(overlay);
    ...
    // 定制RouteOverly
    private class MyDrivingTextPathOverlay extends TextPathOverlay {

        private MyDrivingTextPathOverlay(BaiduMap baiduMap) {
            super(baiduMap);
        }
